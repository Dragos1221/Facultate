...
public class Worker implements Runnable , IObserver {
        ...
        while(connected) {
            try {
               // Object cerere = input.readObject();
                ProjectProto.Request cerere = ProjectProto.Request.parseDelimitedFrom(input);
                login(cerere);
    ...
    public void login(ProjectProto.Request c)
    {
        Oficiu of = new Oficiu(c.getUser().getUsername() , c.getUser().getPasswdord());
        Boolean bol = servOficii.logIn(of,this);
        ProjectProto.Response resp = ProjectProto.Response.newBuilder().setMess("ok").setRBool(bol).build();
        try {
            resp.writeDelimitedTo(output);
            output.flush();
        } catch (IOException e) {
            e.printStackTrace();
        }
